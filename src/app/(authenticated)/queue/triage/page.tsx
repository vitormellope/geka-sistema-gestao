...
export default function TriageQueuePage() {
  ...
  const [actionDemandId, setActionDemandId] = useState<number | null>(null)
  const [pendingObs, setPendingObs] = useState('')
  const [submitting, setSubmitting] = useState(false)

  const loadDemands = useCallback(async () => {
    try {
      const res = await fetch('/api/demands/queue/triage')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Fila de Triagem</h1>
          <p className="text-sm text-slate-500 mt-1">
            {demands.length} demanda{demands.length !== 1 ? 's' : ''} aguardando triagem
          </p>
        </div>
      </div>
      ...
      {demands.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500">Nenhuma demanda na fila de triagem</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Criado em {format(new Date(d.created_at), 'dd/MM/yyyy', { locale: ptBR })}
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button
                    onClick={() => handleRoute(d.id, 'projeto_decupagem')}
                    disabled={submitting}
                    className="px-3 py-1.5 text-xs font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 whitespace-nowrap"
                  >
                    Projeto / Decupagem
                  </button>
                  <button
                    onClick={() => setActionDemandId(actionDemandId === d.id ? null : d.id)}
                    disabled={submitting}
